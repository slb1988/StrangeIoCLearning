...
public class ClientConnection<PacketType> where PacketType : PacketFormat, new()
{
	...
	private void ReceiveCallback(IAsyncResult ar)
	{
		if (this.m_socket.Connected)
		{
			...
			{
				Log.Net.Print(ex.ToString(), new object[0]);
				this.AddDisconnectEvent(BattleNetErrors.ERROR_RPC_PEER_DISCONNECTED);
			}
		}
		else
	...
	public void Update()
	{
		...
		try
		{
			foreach (ClientConnection<PacketType>.ConnectionEvent current in this.m_connectionEvents)
			{
				...
				case ClientConnection<PacketType>.ConnectionEventTypes.OnPacketCompleted:
					...
					{
						IConnectionListener<PacketType> connectionListener = this.m_listeners[k];
