    ...
    class MessageSerializer
    {
        ...
        public void GenerateClassSerializer(ProtoMessage m)
        {
            ...
            else
            {
                ...
                cw.Bracket(m.OptionAccess + " partial " + m.OptionType + " " + m.SerializerType + " : IProtoBuf");
            }
        ...
        void GenerateReader(ProtoMessage m)
        {
            ...
            if (m.OptionType != "interface")
            {
                ...
                cw.Bracket(m.OptionAccess + " void Deserialize(Stream stream)");
        ...
        {
            cw.Summary("Get SerializeSize");
            cw.Bracket("public uint GetSerializedSize()");
            cw.WriteLine("uint num = 0u;");
            cw.WriteLine();
            foreach (Field f in m.Fields.Values)
            {
                fieldSerializer.FieldSizeWriter(m, f, cw, options);
                cw.WriteLine();
            }
            cw.WriteLine("return num;");
            cw.EndBracket();
            ...
            cw.Summary("Serialize this into the stream");
            cw.Bracket(m.OptionAccess + " void Serialize(Stream stream)");
